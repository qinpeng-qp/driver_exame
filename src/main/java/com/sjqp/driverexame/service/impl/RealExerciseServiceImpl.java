...
public class RealExerciseServiceImpl implements RealExerciseService {
    ...
    public ApiResult<List<RealExercise>> getRealExercise(Integer currentPageNo, Integer pageSize) {
        ...
        try {
            if (Objects.nonNull(currentPageNo) && Objects.nonNull(pageSize)) {
                ...
                Condition condition = new Condition(RealExercise.class);
                condition.orderBy("id");
                List<RealExercise> realExerciseList = realExerciseMapper.selectByCondition(condition);
    ...
    @Override
    public ApiResult updateRealExercise(RealExercise realExercise) {
        ApiResult apiResult = new ApiResult(ApiResult.FAIL_RESULT);
        try {
            int row = realExerciseMapper.updateByPrimaryKeySelective(realExercise);
            if (row > 0) {
                apiResult.setData(ApiResult.SUCCESS_RESULT);
                apiResult.setMsg("修改成功");
            }
        } catch (Exception e) {
            logger.error("updateRealExercise e{}", e);
        }
        return apiResult;
    }
